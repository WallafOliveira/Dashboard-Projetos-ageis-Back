...
class OperacionalSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // 3. Pedidos & Financeiro Transacoes
        $statusPedido = ['Em Andamento', 'Concluído', 'Concluído', 'Concluído', 'Concluído', 'Pendente', 'Cancelado'];
        for ($i = 0; $i < 200; $i++) {
            $dataPedido = $faker->dateTimeBetween('-1 year', 'now');
            $status = $faker->randomElement($statusPedido);
            
            // Garante que alguns pedidos sejam concluídos hoje
            if ($i < 10 && $i % 3 === 0) {
                $status = 'Concluído';
                $dataConclusao = Carbon::today();
            } elseif ($status === 'Concluído') {
                $dataConclusao = (clone $dataPedido)->modify('+' . rand(1, 15) . ' days');
            } else {
                $dataConclusao = null;
            }

            $valorTotal = $faker->randomFloat(2, 500, 15000);
            $custoTotal = $valorTotal * $faker->randomFloat(2, 0.4, 0.8);
